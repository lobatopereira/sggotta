...
def DistRequestProductDelivered(request,hashid):
	...
	with transaction.atomic():
		if request.method == "GET":
			data_distribui = request.GET.get('data_distribui')
			if data_distribui is None or data_distribui == "":
				...
				for x in pediduprodutuData:
					...
					obj2.save()
					ProductSell.objects.create(client=objects1.client,product=obj2.product,unit=obj2.unit,date=data_distribui,total_out=obj2.totalprodutu,totalprice=obj2.totalpresu,user_created=request.user)
					...
					ProductTransaction.objects.create(date=data_distribui,total_in=0,total_out=obj2.totalprodutu,total=totalupdated,user_created=request.user,unit=productunit,product=obj2.product)
